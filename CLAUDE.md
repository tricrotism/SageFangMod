...
- Resource leaks — event listeners that outlive their module, `Timer`/scheduled tasks left running, network receivers
  not cleaned up, ImGui state never reset. Modules toggle via `onActivate`/`onDeactivate` — make sure deactivate undoes
  what activate did.
...
- Per-frame waste in ImGui `frame(ImGuiIO)` — it runs every frame. Don't allocate, parse, format, or do registry lookups
...
- Misleading identifiers (`get…` that mutates, `active` vs `visible` confusion — they mean different things here).
...
These are intentional in this codebase — don't flag: Lombok `@Getter`/`@Setter` field access, the
...
For non-trivial changes, blank line then a 1–2 short paragraph body:

1. **Problem** — concrete: name the class/mixin/module, the symptom, the real impact ("runClient crashed in
...
`MiniMessage.format(String)` is cached (15s, for hot repeats); the var-args overload is not — prefer it only when you
...
- Modules **self-register** into `Module.REGISTRY` on construction — the settings UI discovers them dynamically. Don't
...
- State is config-backed: `active` (`module.<id>.enabled`) and `visible` (`module.<id>.visible`). Toggle through
  `toggle()` / `toggleVisible()` — they persist automatically. Don't set those keys by hand.
...
- Write with `Config.setProperty(...)`; the persistence layer handles the file. Never write `sagefang.properties`
  yourself.
- `Config.onChange(...)` is already wired for the dirty flag — don't add competing writers.
...
- `./gradlew runClient` — launch the dev client (DevAuth handles login). `./gradlew build` to produce the jar.
...
- Bundled deps (imgui, cloud, adventure, avaje, checker-qual) ship as jar-in-jar via loom `include` — no shadow/fat jar.
...
- Third-party Fabric deps must be built for non-obf 26.1 (official names). A dep compiled against intermediary (
  `net.minecraft.class_XXXX`) will throw `NoClassDefFoundError` at runtime — bump it to a 26.1 build. See the project
